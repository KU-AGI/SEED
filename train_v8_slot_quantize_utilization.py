...
class SlotTrainingWrapper(LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        quant, embed_ind, loss_codebook = self.quantize(slots_down)
        # assert (quant == self.quantizer.indices_to_codes(embed_ind)).all()
        quant_inf = self.quantize.get_output_from_indices(embed_ind)

        # embed_ind = embed_ind.reshape(slots.shape[0], -1)

        self.slots_ind.append(embed_ind)

        return

    def on_test_epoch_end(self):
        import pandas as pd
        flatten_slots_ind = torch.concat(self.slots_ind, dim=0).flatten().cpu().numpy()
        unique, counts = np.unique(flatten_slots_ind, return_counts=True)

        df = pd.DataFrame({'Value': unique, 'Count': counts})
        df_sorted = df.sort_values(by='Count', ascending=False).reset_index(drop=True)

        all_values = pd.DataFrame({'Value': np.arange(0, self.cfg.stage1.vq.n_embed)})
        df_full = pd.merge(all_values, df_sorted, on='Value', how='left')
        
        df_full_sorted = df_full.sort_values(by='Count', ascending=False).reset_index(drop=True)
        df_full_sorted.to_csv(f"{self.save_path}/codebook_distribution.csv", index=False)
        return
