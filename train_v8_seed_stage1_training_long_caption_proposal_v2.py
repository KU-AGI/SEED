...
class SEEDTrainingWrapper(LightningModule):
    ...
    def get_stage_1_loss_compression_contrastive(self, batch, batch_idx: int):
        """
        ...
        # text_token shape is [b, max_length]
        ...
        # text_output shape is [b, max_length, 768]
        text_output = self.image_tokenizer.model.Qformer.bert(
            text_tokens.input_ids.to(device),
            attention_mask=text_tokens.attention_mask.to(device),
            return_dict=True,
        ...
        sim_i2t = sim_q2t[:, :, cur_training_token]
        sim_i2t = sim_i2t / self.temp
        ...
        sim_t2i = sim_t2q[:, :, cur_training_token]
...
if __name__ == "__main__":
    ...
    # checkpoint_path = "/home/zheedong/Projects/SEED/logs/seed_stage_1_long_caption_training_debug/lightning_logs/version_81_token_num_4_resume_from_80/checkpoints/epoch=0-step=54.ckpt"
    # wrapper = SEEDTrainingWrapper.load_from_checkpoint(checkpoint_path, cfg=cfg).to(device)
    wrapper.setup("fit")
