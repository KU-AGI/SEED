...
from models.seed_qformer.qformer_quantizer import VectorQuantizer2
...
class SEEDTrainingWrapper(LightningModule):
    ...
    def setup(self, stage):
        ...
        if self.stage == 1:
            ...
            if hasattr(self.cfg.stage2, "init_stage_2") and not self.cfg.stage2.init_stage_2:
    ...
    def random_initialize_stage2_model_weights(self):
        ...
        self.image_tokenizer.model.encode_task_layer = nn.Sequential(
            nn.Linear(768, 768),
            nn.Tanh(),
            nn.Linear(768, 32)
        )
        self.image_tokenizer.model.quantize = VectorQuantizer2(
            n_e=8192,
            e_dim=32,
            beta=0.25,
            remap=None,
        ...
        self.image_tokenizer.model.decode_task_layer = nn.Sequential(
            nn.Linear(32, 768),
            nn.Tanh(),
            nn.Linear(768, 768)
        )
        self.image_tokenizer.model.pos_embed = nn.Parameter(torch.randn(1, 32, 768))
        self.image_tokenizer.model.blocks = nn.ModuleList([
            Block(dim=768,
                  num_heads=12,
                  mlp_ratio=4.0,
                  qkv_bias=True,
                  ...
                  attn_drop=0.0,
                  drop_path=0.0,
                  norm_layer=partial(nn.LayerNorm, eps=1e-6)) for i in range(4)
        ])

        self.image_tokenizer.model.pos_embed_image = nn.Parameter(torch.randn(1, 32, 768))
        self.image_tokenizer.model.blocks_image = nn.ModuleList([
            Block(dim=768,
                  num_heads=12,
                  mlp_ratio=4.0,
                  qkv_bias=True,
        ...
        self.image_tokenizer.model.image_down = nn.Sequential(
        ...
        self.image_tokenizer.model.distill_image_proj = nn.Linear(32 * 32, 1024)
    ...
    def get_stage_2_loss(self, batch, batch_idx: int, is_validation=False):
        ...
        if bypass_codebook:
            ...
            if is_validation:
                print("Quantize")
            if self.cfg.stage2.vq.type != "residual_vq":
                ...
                logging_dict["loss_codebook"] = loss_embed
                logging_dict["sim_32"] = F.cosine_similarity(query_output_down, quant, dim=-1).mean()
                ...
                # In case of residual_vq
                quant, embed_ind, loss_embed = self.image_tokenizer.model.quantize(query_output_down)
                perplexity = None
                loss_embed = loss_embed.sum()
                # uggly solution to avoid the error
                num_replaced_codebook = -1

            if is_validation:
                self.seed_ind.append(embed_ind)
        ...
        if self.recon_s:
            ...
            logging_dict["sim_768"] = F.cosine_similarity(recon_s, causal_embeddings, dim=-1).mean()
        ...
        logging_dict["loss_generation_embed"] = loss_generation_embed
    ...
    def test_step(self, batch, batch_idx: int):
        with torch.no_grad():
            self.get_stage_2_loss(batch, batch_idx, is_validation=True)
        return
    ...
    def calculate_perplexity(self, embed_ind):
        n_embed = 8192
    ...
    def on_validation_end(self):
        n_embed = 8192
        flatten_seed_ind = torch.cat(self.seed_ind, dim=0)
        # Uggly solution, don't use quant argument
        perplexity = self.calculate_perplexity(embed_ind=flatten_seed_ind)
        self.logger.experiment.add_scalar("total_perplexity", perplexity, global_step=self.global_step)

        flatten_seed_ind = flatten_seed_ind.cpu().numpy()
        unique, counts = np.unique(flatten_seed_ind, return_counts=True)
        unused_code_count = n_embed - len(unique)
        self.logger.experiment.add_scalar("unused_code_count", unused_code_count, global_step=self.global_step)
        self.logger.experiment.add_scalar("unused_code_percentage", unused_code_count / n_embed * 100, global_step=self.global_step)
        self.print(f"Unused code count: {unused_code_count}")

        sorted_indices = np.argsort(counts)
        top_10 = sorted_indices[-10:]
        top_10_ratio = counts[top_10].sum() * 100 / counts.sum()
        self.logger.experiment.add_scalar("top_10_ratio_percentage", top_10_ratio, global_step=self.global_step)
        self.logger.experiment.add_scalar("top_used_code", unique[sorted_indices[-1]], global_step=self.global_step)
        self.print(f"Top 10 ratio: {top_10_ratio.sum()}")
        return
