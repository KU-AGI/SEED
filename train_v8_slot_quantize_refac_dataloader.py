...
from datamodules.datamodule_refac import MainDataModule
...
class SlotTrainingWrapper(LightningModule):
    ...
    def get_quantize_loss(self, batch, batch_idx: int, is_validation=False):
        ...
        with torch.no_grad():
            slots = self.get_slot_embedding(batch, batch_idx, is_validation=is_validation)
        ...
        # DEBUG : Without transformer blocks
        ...
        # Combine loss
        if self.cfg.stage1.unfreeze_linear:
            loss = loss_codebook + 4 * loss_recon + loss_diffusion
        else:
            # Don't use loss_diffusion
            loss = loss_codebook + 4 * loss_recon
...
if __name__ == "__main__":
    ...
    from datamodules.datapipe_wds import get_dataloader
    dataloader = get_dataloader()

    cfg.experiment.total_training_steps = 10000000
    ...
    if cfg.resume:
        ...
        trainer.fit(
            wrapper, train_dataloaders=dataloader, val_dataloaders=dataloader
        )
