...
class SlotTrainingWrapper(LightningModule):
    ...
    def __init__(self, cfg):
        ...
        if hasattr(self.cfg.stage1, 'slot_num') and self.cfg.stage1.slot_num is not None:
            self.slot_num = self.cfg.stage1.slot_num
        ...
        if hasattr(self.cfg.stage1, "itc_weight") and \
            ...
            self.image_feat_proj = nn.Linear(768, 1024)
        ...
        if self.stage == 2:
            ...
            if self.cfg.stage2.vq.vq_type == "vq":
                ...
                raise ValueError(f"Unknown vq_type {self.cfg.stage2.vq.vq_type}")

            self.pos_embed = nn.Parameter(torch.zeros(1, self.slot_num, slot_embed))
            ...
            if self.cfg.stage2.use_blocks_image and \
                ...
                self.pos_embed_image = nn.Parameter(torch.zeros(1, self.slot_num, slot_embed))
    ...
    def get_itc_loss_use_last_token(self, batch, batch_idx: int, is_validation=False, image_feats=None):
        ...
        image_feats = F.normalize(image_feats, dim=-1) # [b, 32, 1024]
        ...
        ###========= All tokens =========###
        '''
        sim_i2t_all_tokens = sim_q2t.max(-1)[0]
        sim_i2t_all_tokens = sim_i2t_all_tokens / self.temp

        sim_t2i_all_tokens = sim_t2q.max(-1)[0]
        sim_t2i_all_tokens = sim_t2i_all_tokens / self.temp

        loss_itc_all = (
                           F.cross_entropy(sim_i2t_all_tokens, targets, label_smoothing=0.1)
                           + F.cross_entropy(sim_t2i_all_tokens, targets, label_smoothing=0.1)
                   ) / 2

        ###========= Mean =========###
        # Mean aggregate across all slot tokens
        sim_i2t_mean = sim_q2t.mean(-1)
        sim_i2t_mean = sim_i2t_mean / self.temp

        sim_t2i_mean = sim_t2q.mean(-1)
        sim_t2i_mean = sim_t2i_mean / self.temp

        loss_itc_mean = (
                            F.cross_entropy(sim_i2t_mean, targets, label_smoothing=0.1)
                            + F.cross_entropy(sim_t2i_mean, targets, label_smoothing=0.1)
                     ) / 2
        '''
        # return 0.5 * loss_itc + 0.25 * loss_itc_all + 0.25 * loss_itc_mean
        return loss_itc
...
if __name__ == "__main__":
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[ModelSummary(max_depth=3), lr_logger, checkpoint_callback, device_stats],
