...
def clip_score(compare_img_set: tuple, clip_set: tuple, device):
    ...
    images_1 = [preprocess_clip(Image.fromarray(np.array(rearrange(pixel, 'c h w -> h w c')))) for pixel in pixel_1]
    images_1 = torch.stack(images_1, dim=0).to(device)
    images_2 = [preprocess_clip(Image.fromarray(np.array(rearrange(pixel, 'c h w -> h w c')))) for pixel in pixel_2]
