...
class SEEDTrainingWrapper(LightningModule):
    ...
    def forward_stage_2(self, batch, batch_idx: int, bypass_codebook=False):
        ...
        return reverse_output_proj, embed_ind
    ...
    def test_step(self, batch, batch_idx: int):
        ...
        with torch.no_grad():
            image_embeds, embed_ind = self.forward_stage_2(batch, batch_idx, False)
            self.seed_ind.append(embed_ind)

    def test_epoch_end(self, outputs: List[Any]):
        import pandas as pd
        flatten_seed_ind = torch.cat(self.seed_ind, dim=0).flatten().cpu().numpy()
        unique, counts = np.unique(flatten_seed_ind, return_counts=True)
        
        df = pd.DataFrame({'Value': unique, 'Count': counts})
        df_sorted = df.sort_values(by='Count', ascending=False).reset_index(drop=True)

        all_values = pd.DataFrame({'Value': np.arange(0, 8192)})
