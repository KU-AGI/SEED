...
class SlotTrainingWrapper(LightningModule):
    ...
    def __init__(self, cfg):
        ...
        # Added
        self.slot_layernorm = nn.LayerNorm(768)
        self.slot_1024 = nn.Linear(768, 1024)
        ...
        self.encode_task_layer = nn.Sequential(
            nn.Linear(768, 768),
            nn.Tanh(),
            nn.Linear(768, codebook_embed_dim)  # for quantize
        )
        ...
        self.decode_task_layer = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Linear(codebook_embed_dim, 768)  # for quantize
        )
    ...
    def setup(self, stage):
        ...
        # Freeze out layer norm and out linear
    ...
    def get_quantize_loss(self, batch, batch_idx: int, is_validation=False):
        ...
        with torch.no_grad():
            slots = self.get_slot_embedding(batch, batch_idx)
            gt_slots_1024 = self.out_linear_1024(slots)
        ...
        if self.cfg.stage1.use_blocks_image:
            ...
            slots_up_768_blocks_image_applied = self.slot_layernorm(slots_up_768_blocks_image_applied)
            slots_1024 = self.slot_1024(slots_up_768_blocks_image_applied)
        else:
            slots_1024 = self.slot_1024(slots_up_768_blocks_applied)

        # diffusion loss just for logging
        ...
        loss_slot_1024_mse = F.mse_loss(gt_slots_1024, slots_1024, reduction="mean")
        logging_dict["loss_slot_1024_mse"] = loss_slot_1024_mse
        ...
        if hasattr(self.cfg.stage1.vq, "vq_type") and \
            ...
            loss = self.cfg.stage1.loss_weight.loss_codebook * loss_codebook + \
                    ...
                    self.cfg.stage1.loss_weight.loss_slot_mse * loss_slot_1024_mse
                    # self.cfg.stage1.loss_weight.loss_diffusion * loss_diffusion
...
if __name__ == "__main__":
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[ModelSummary(max_depth=2), lr_logger], #,checkpoint_callback],
